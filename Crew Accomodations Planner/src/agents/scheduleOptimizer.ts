...
export async function scheduleOptimizer(cands: HotelCandidate[], _ctx: AgentContext, push:(r:DecisionRecord)=>void){
  const scored = cands.map(h=>({ h, s: scoreHotel(h) })).sort((a,b)=>b.s-a.s);
