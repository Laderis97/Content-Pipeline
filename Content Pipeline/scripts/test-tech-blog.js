...
// Load tech blog configuration
const configPath = path.join(__dirname, '..', 'config', 'sites', 'tech-blog.json');

if (!fs.existsSync(configPath)) {
  console.error('❌ Tech blog configuration not found at:', configPath);
  process.exit(1);
}

const siteConfig = JSON.parse(fs.readFileSync(configPath, 'utf8'));

console.log('💻 Testing Technology Blog Connection');
console.log('=====================================');
...
if (!siteConfig.appPassword) {
  ...
  console.log('Run: scripts/setup-tech-blog.ps1 to set up the Application Password');
  process.exit(1);
}
...
async function testContentPublishing() {
  ...
  const testPost = {
    ...
    content: '<p>This is a test post to verify the technology blog is working correctly.</p><p>Topics: programming, AI, software development, cybersecurity</p>',
    excerpt: 'Test post for tech blog verification',
...
async function runTests() {
  try {
    ...
    console.log('\n🎉 Tech blog is fully configured and working!');
    console.log('\n📊 Site Summary:');
    ...
    if (error.message.includes('401') || error.message.includes('403')) {
      ...
      console.log('   Run: scripts/setup-tech-blog.ps1');
